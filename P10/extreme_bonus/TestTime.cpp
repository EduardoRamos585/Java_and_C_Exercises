...
int main(int argc, char* argv[]) {
    ...
    vector <<=1; // bonus
     int seconds = 600;
     Time new_time{9, 30, 0};
    if(fails(new_time + seconds, "09:40:00")) result |= vector;
    if(fails(seconds + new_time, "09:40:00")) result |= vector;
   // --------------------------------------------------------------------------
    vector <<=1; //extreme_bonus
	
     Time extreme_time{9, 30, 0};
     int hour = extreme_time[0]; // hour is 9
     int minute = extreme_time[1]; // minute is 30
     int second = extreme_time[2]; // second is 0
			  
     if(hour != 9) result |= vector;
     if(minute != 30) result |= vector;
     if(second != 0) result |= vector;
   //---------------------------------------------------------------------------
